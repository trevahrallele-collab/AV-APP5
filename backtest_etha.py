...
def main():
    ...
    print("Fetching ETHA ETF data...")
    try:
        df = fetcher.fetch_etf_data("ETHA")
        result = simple_backtest(df)
        
        if "error" in result:
            print(f"Error: {result['error']}")
        else:
            save_to_cache(df, result)
            summary = result['summary']
            print(f"\n=== ETHA ETF BACKTEST RESULTS ===")
